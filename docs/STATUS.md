...
- `python play.py --perf-sentinel --profile-on-lag --lag-frame-ms 50` (manual F10 perf-sample verification required on a machine with `pygame`; blocked in this container: `ModuleNotFoundError: No module named 'pygame'`)
- `python play.py --visual-audit` (manual visual-audit runtime verification required on a machine with `pygame`; blocked in this container: `ModuleNotFoundError: No module named 'pygame'`)
...
## Phase
- **Current phase:** **Playable Core Loop Slice — Campaign Travel → Contact → Local Encounter → Combat → Extraction/Return**.
- **Next action:** Run `python play.py --visual-audit` on a pygame-enabled machine and confirm `first_attack`/`combat_result` are `ok` with `combat_cue_diagnostics.cue_rendered=true` and visible cue bboxes in the contact sheet.
...
## Phase
- **Current phase:** **Playable Core Loop Slice — Campaign Travel → Contact → Local Encounter → Combat → Extraction/Return**.
- **Next action:** Re-run the two `play.py` runtime commands on Windows with `pygame` installed to generate a populated lag capture and confirm Gate 1 diagnosis from live timings.
