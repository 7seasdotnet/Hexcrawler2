...
from hexcrawler.sim.site_pressure import (
    ...
    SITE_PRESSURE_BRIDGE_OUTCOME_EVENT_TYPE,
    SitePressureBridgeModule,
...
def _sim(seed: int = 123) -> Simulation:
    ...
    sim.register_rule_module(SitePressureBridgeModule())
...
def test_a4_claim_opportunity_consumed_bridges_into_site_pressure_apply() -> None:
    sim = _sim()
    ...
    bridge_outcome = _events(sim, SITE_PRESSURE_BRIDGE_OUTCOME_EVENT_TYPE)[-1]
    assert bridge_outcome["params"]["outcome"] == "emitted"

    apply_events = _events(sim, SITE_PRESSURE_APPLY_EVENT_TYPE)
...
def test_a4_missing_context_skips_without_mutation() -> None:
    sim = _sim()
    sim.schedule_event_at(
        tick=0,
        event_type=CLAIM_OPPORTUNITY_CONSUMED_EVENT_TYPE,
    ...
    assert _events(sim, SITE_PRESSURE_APPLY_EVENT_TYPE) == []
    assert sim.state.world.sites["camp_01"].site_state.pressure_records == []


def test_a4_rules_state_idempotence_ledger_prevents_duplicate_emission() -> None:
    sim = _sim()
    ...
    sim.set_rules_state(SitePressureBridgeModule.name, {"processed_source_event_ids": [source_event_id]})

    sim.advance_ticks(1)

    bridge_outcome = _events(sim, SITE_PRESSURE_BRIDGE_OUTCOME_EVENT_TYPE)[-1]
    assert bridge_outcome["params"]["outcome"] == "skipped_duplicate"
    assert _events(sim, SITE_PRESSURE_APPLY_EVENT_TYPE) == []
    assert sim.state.world.sites["camp_01"].site_state.pressure_records == []


def test_a4_multiple_source_events_preserve_stable_order() -> None:
    sim = _sim()
    sim.schedule_event_at(
        tick=0,
        event_type=CLAIM_OPPORTUNITY_CONSUMED_EVENT_TYPE,
...
def test_a4_bridge_path_save_load_and_hash_stability(tmp_path) -> None:
    ...
    save_path = tmp_path / "site_pressure_bridge_a4.json"
    save_game_json(save_path, split.state.world, split)

    _, loaded = load_game_json(save_path)
    loaded.register_rule_module(SitePressureBridgeModule())
    loaded.register_rule_module(SitePressureMutationModule())
